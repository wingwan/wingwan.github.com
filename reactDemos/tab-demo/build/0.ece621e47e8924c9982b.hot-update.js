	...
	var TabSelector = React.createClass({displayName: "TabSelector",
	    ...
	    render: function() {
	        var tabs = this.props.data.map(function (item) {
	            ...
	            return React.createElement("li", {"data-value": item.value, className: selected, onClick: this.handleOnClick}, item.name);
	        }, this);

	        var cnts = this.props.data.map(function(item){
	            var active = item.value == this.state.selected ? 'active' : 'hide';
	            return React.createElement("li", {"data-value": item.value, className: active}, item.content);
	        }, this);
